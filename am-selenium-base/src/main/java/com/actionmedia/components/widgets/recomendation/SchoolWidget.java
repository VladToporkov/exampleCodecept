...
public class SchoolWidget extends Widget {
    ...
    @FindBy(id = "widget-school")
    @Wait
    @ErrorMessage("Виджет Высшая Школа не найден")
    @CacheLookup
    private WebElement widget;

    @Step("Проверяется, что виджет Школа отображается")
    public void checkWidgetIsPresent() {
        postponedAssertTrue(widget.isEnabled() && widget.isDisplayed(), "Виджет Школа не отображается");
    }

    @Step("Проверяется, что виджет Школа не отображается")
    public void checkWidgetIsNotPresent() {
        WebElement widget = findElementByNoThrow(By.id("widget-school"));
        postponedAssertTrue(widget == null || !widget.isDisplayed(), "Виджет Школа отображается");
    }
}
