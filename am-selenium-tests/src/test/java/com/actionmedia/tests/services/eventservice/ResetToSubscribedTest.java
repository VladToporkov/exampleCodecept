 ...
 * Time: 16:18
 */
public class ResetToSubscribedTest extends EventServiceBaseTest {
    ...
    public void dropEvent_ResetToSubscribed_success_test() {
        report("Проверяется общая структура ответного xml для метода 'DropEvent' для события ResetToSubscribed");
        ...
        report("Событие - ResetToSubscribed");
        ...
        try {
            ...
            checkUnSubscribeSuccessResponse(response, "ResetToSubscribed");
            postponedAssertFalse(isUnSubscribeListContainsUserToken(SUBSCRIBE_USER_TOKEN), "Присутсвует запись в t_queue.UNSUBSCRIBES для USER_TOKEN " + SUBSCRIBE_USER_TOKEN);
        } catch (ServiceException e) {
    ...
    @Test(groups = {event, service})
    public void dropEvent_ResetToSubscribed_incorrect_test() {
        report("Проверяется общая структура ответного xml для метода 'DropEvent' для события ResetToSubscribed. Некорректный subscribe_key");

        String subscribeKey = getRandomString(32);
