 ...
 * Time: 22:30
 */
public class ServiceContents_NPD_ATTACHES_TXT_Tests extends ServiceContentsBaseTest {
    ...
    public void add_txt_document_npd_attaches_test() throws IOException {
        ...
        report("Тест для проверки добавления txt-файла в хранилище npd_attaches, раздел по-умолчанию");
        ...
        WebResource webResource = client.resource(URL + "storage/npd_attaches");
    ...
    public void add_new_version_txt_document_npd_attaches_test() throws IOException {
        JSONObject json = new JSONObject();
        ...
        report("Тест для проверки добавления новой версии txt-файла в хранилище npd_attaches");
        report("filename - " + json.get("filename"));
        report("mimeType - " + json.get("mimeType"));
        report("etag - " + json.get("etag"));
        report("data - " + json.get("data"));
    ...
    public void get_txt_document_info_test() throws IOException {
        report("Тест для проверки получения мета-описания txt-файла в хранилище npd_attaches");
        report("filename - " + TXT_FILE_NAME);

        Client client = Client.create();
        WebResource webResource = client.resource(URL + "storage/npd_attaches/info/" + TXT_FILE_NAME);
    ...
    public void get_txt_document_file_test() throws IOException {
        report("Тест для проверки получения контента последней версии txt-файла в хранилище npd_attaches");
        report("filename - " + TXT_FILE_NAME);

        Client client = Client.create();
        WebResource webResource = client.resource(URL + "storage/npd_attaches/file/" + TXT_FILE_NAME);
        ClientResponse response = webResource.type(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON).get(ClientResponse.class);
    ...
    public void get_txt_document_file_version_test() throws IOException {
        report("Тест для проверки получения контента версии txt-файла в хранилище npd_attaches");
        report("filename - " + TXT_FILE_NAME);
        report("etag - " + UUID_DEFAULT);

        Client client = Client.create();
        WebResource webResource = client.resource(URL + "storage/npd_attaches/file/" + TXT_FILE_NAME + "/" + UUID_DEFAULT);
        ClientResponse response = webResource.type(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON).get(ClientResponse.class);
    ...
    public void get_txt_document_not_included_file_version_test() throws IOException {
        ...
        report("Тест для проверки получения контента несуществующей версии txt-файла в хранилище npd_attaches");
        report("filename - " + TXT_FILE_NAME);
        report("etag - " + uuid);

        Client client = Client.create();
        WebResource webResource = client.resource(URL + "storage/npd_attaches/file/" + TXT_FILE_NAME + "/" + uuid);
        ClientResponse response = webResource.type(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON).get(ClientResponse.class);
    ...
    public void delete_txt_document_file_version_test() throws IOException {
        report("Тест для проверки удаления версии txt-файла в хранилище npd_attaches");
        report("filename - " + TXT_FILE_NAME);
        report("etag - " + UUID_DEFAULT);

        Client client = Client.create();
        WebResource webResource = client.resource(URL + "storage/npd_attaches/file/" + TXT_FILE_NAME + "/" + UUID_NEW);
    ...
    public void delete_txt_document_file_test() throws IOException {
        report("Тест для проверки удаления txt-файла в хранилище npd_attaches");
        report("filename - " + TXT_FILE_NAME);
