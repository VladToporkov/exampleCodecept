 ...
 * Time: 10:04
 */
public class StartPagesMailSenderListener implements ISuiteListener, ITestListener {
    ...
    private static Map<String, List<String>> checkedDocsForSystem = new HashMap<String, List<String>>();
    private static Map<String, Map<String, List<String>>> checkedDocsMapForSystem = new HashMap<String, Map<String, List<String>>>();
    ...
    public void onFinish(ISuite suite) {
        if (counter == 0) {
            ...
            String systemName = "";
            String methodName = suite.getAllMethods().get(0).getTestClass().getName();
            ...
            if (result.toString().isEmpty()) {
                ...
                sendEmail(mailProperties, result.toString() + (isStartPagesTests ? getTextForSuccessResultForStartPages() : getTextForSuccessResult()), systemName, systemName + ". Отсутствует контент. " + typeCheck);
            }
            counter++;
        }
    }
    ...
    private void sendEmail(final Properties mailProperties, String text, String systemName, String subject) {
        ...
        if (isBackLinksTests) {
            ...
            to = StringUtils.join(new HashSet<String>(Arrays.asList(to.split(","))), ",");
        }
    ...
    private String getTextForSuccessResult() {
        ...
        for (Map.Entry<String, List<String>> entry : checkedDocsForSystem.entrySet()) {
    ...
    private String getTextForSuccessResultForStartPages() {
        ...
        for (Map.Entry<String, Map<String, List<String>>> entry : checkedDocsMapForSystem.entrySet()) {
            String systemName = entry.getKey();
            int count = 0;
    ...
    public void onTestFailure(ITestResult testResult) {
        ...
        checkedDocsForSystem.put(pair.getLeft().getDescription(), pair.getRight());
    ...
    public void onTestSuccess(ITestResult testResult) {
        ...
        if (isTestVerifyStartPages(testResult.getMethod())) {
            checkedDocsMapForSystem.put(pairMap.getLeft().getDescription(), pairMap.getRight());
        }
    }
