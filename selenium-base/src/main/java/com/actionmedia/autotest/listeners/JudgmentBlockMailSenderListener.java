 ...
 * Time: 10:04
 */
public class JudgmentBlockMailSenderListener implements ISuiteListener, ITestListener {
    ...
    public void onTestFailure(ITestResult testResult) {
        if (!isVerifyJudgmentBlockTest(testResult.getMethod())) {
            return;
        }
    ...
    private void sendEmail(final Properties mailProperties, String text, List<File> attachFileList, String systemName) {
        ...
        String to = mailProperties.getProperty("mail.to.judgment");

        String[] recipientArr = to.split(",");

        String subject = systemName + ". Проверка блока 'Судебные решения'.";
    ...
    private boolean isVerifyJudgmentBlockTest(ITestNGMethod method) {
        for (String group : method.getGroups()) {
            if (group.contains(judgment)) {
                return true;
            }
        }
        return false;
